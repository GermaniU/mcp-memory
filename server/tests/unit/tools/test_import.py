...
async def test_import_happy_path(store):
    jsonl = "\n".join([
    ...
    result = await import_memories(ImportInput(jsonl=jsonl), store=store)
    assert result.imported == 2
    assert result.skipped == 0
    assert result.errors == []


async def test_import_namespace_override(store):
...
async def test_import_collision_skips_existing_id(store):
...
async def test_import_malformed_line_accumulates_error(store):
    jsonl = "\n".join([
...
async def test_import_missing_required_field_accumulates_error(store):
    ...
    result = await import_memories(ImportInput(jsonl=bad), store=store)
    assert result.imported == 0
    assert len(result.errors) == 1
    assert result.errors[0].line == 1


async def test_import_rejects_payload_over_size_limit(store):
    ...
    with pytest.raises(ValueError, match="límite permitido"):
        await import_memories(ImportInput(jsonl=oversized), store=store)


async def test_import_accepts_payload_at_size_limit(store):
    ...
    result = await import_memories(ImportInput(jsonl=at_limit), store=store)
    assert result.imported == 0  # no hay líneas JSONL válidas, todo es un solo "x" continuo


async def test_import_blank_lines_ignored(store):
    jsonl = "\n".join([
        "",
...
async def test_import_preserves_original_timestamps_and_tags(store):
    ...
    await import_memories(ImportInput(jsonl=jsonl), store=store)
